...
def generation(data):
    ...
    if character_name in ['蛍', '空']:
        character_image = Image.open(
            f'{cwd}/character/{character_name}({element})/avatar.png').convert("RGBA")
    else:
        if character_costume:
            character_image = Image.open(
                f'{cwd}/character/{character_name}/{character_costume}.png').convert("RGBA")
        else:
            character_image = Image.open(
                f'{cwd}/character/{character_name}/avatar.png').convert("RGBA")
    ...
    if character_name == 'アルハイゼン':
        character_avatar_mask2 = Image.open(
            f'{cwd}/Assets/Alhaitham.png').convert('L').resize(character_image.size)
    else:
        character_avatar_mask2 = Image.open(
            f'{cwd}/Assets/CharacterMask.png').convert('L').resize(character_image.size)
    character_image.putalpha(character_avatar_mask2)

    character_paste = Image.new("RGBA", base_image.size, (255, 255, 255, 0))

    character_paste.paste(character_image, (-160, -45),
                          mask=character_avatar_mask)
    ...
    # 武器
    weapon = Image.open(
        f'{cwd}/weapon/{weapon_name}.png').convert("RGBA").resize((128, 128))
    ...
    weapon_r_image = Image.open(
        f'{cwd}/Assets/Rarelity/{weapon_reality}.png').convert("RGBA")
    ...
    talent_base = talent_base.resize(
        (int(talent_base.width / 1.5), int(talent_base.height / 1.5)))

    for i, t in enumerate(['通常', 'スキル', "爆発"]):
        talent_paste = Image.new("RGBA", talent_base.size, (255, 255, 255, 0))
        talent = Image.open(
            f'{cwd}/character/{character_name}/{t}.png').resize((50, 50)).convert('RGBA')
        talent_mask = talent.copy()
        talent_paste.paste(
            talent,
            (talent_paste.width // 2 - 25,
             talent_paste.height // 2 - 25),
            mask=talent_mask)
    ...
    for c in range(1, 7):
        if c > character_constellations:
            star_paste.paste(star_locked, (666, -10 + c * 93),
                             mask=star_locked_mask)
        else:
            chara_star = Image.open(
                f'{cwd}/character/{character_name}/{c}.png').convert("RGBA").resize((45, 45))
            chara_star_paste = Image.new(
                "RGBA", star_base.size, (255, 255, 255, 0))
            chara_star_mask = chara_star.copy()
            chara_star_paste.paste(chara_star,
                                   (int(chara_star_paste.width / 2) - 25,
                                    int(chara_star_paste.height / 2) - 23),
    ...
    level_length = drew_base.textlength(
        "Lv." + str(character_level), font=config_font(25))
    drew_base.text((35, 75), "Lv." +
                   str(character_level), font=config_font(25))
    if friend_ship:
        friendshiplength = drew_base.textlength(
            str(friend_ship), font=config_font(25))
        drew_base.rounded_rectangle(
            (35 +
             level_length +
             5,
             74,
            ...
            radius=2,
            fill="black")
        friend_shipIcon = Image.open(f'{cwd}/Assets/Love.png').convert("RGBA")
        friend_shipIcon = friend_shipIcon.resize(
            (int(friend_shipIcon.width * (24 / friend_shipIcon.height)), 24))
        Fmask = friend_shipIcon.copy()
        base_image.paste(
            friend_shipIcon, (42 + int(level_length), 76), mask=Fmask)
        drew_base.text((73 + level_length, 74),
                       str(friend_ship), font=config_font(25))

    drew_base.text((42, 397), f'Lv.{character_talent["通常"]}', font=config_font(
        17), fill='aqua' if character_talent["通常"] >= 10 else None)
    drew_base.text((42, 502), f'Lv.{character_talent["スキル"]}', font=config_font(
        17), fill='aqua' if character_talent["スキル"] >= 10 else None)
    drew_base.text((42, 607), f'Lv.{character_talent["爆発"]}', font=config_font(
        17), fill='aqua' if character_talent["爆発"] >= 10 else None)
    ...
    for k, v in character_status.items():
        ...
        if k not in disper:
            ...
            drew_base.text((1360 - state_len, 67 + i * 70),
        ...
        if k in ['HP', '防御力', '攻撃力']:
            ...
            drew_base.text((1360 - hp_size, 97 + i * 70), hp_pls,
                           fill=(0, 255, 0, 180), font=config_font(12))
            drew_base.text(
                (1360 - hp_size - hp_bsize - 1,
                 97 + i * 70),
                hp_base,
                font=config_font(12),
                fill=(
    ...
    wlebel_len = drew_base.textlength(
        f'Lv.{weapon_level}', font=config_font(24))
    drew_base.rounded_rectangle(
        (1582, 80, 1582 + wlebel_len + 4, 108), radius=1, fill='black')
    ...
    drew_base.text(
        (1623, 120), f'基礎攻撃力  {weapon_base_atk}', font=config_font(23))
    ...
    if weapon_sub_op_key is not None:
        base_atk = Image.open(
            f'{cwd}/emotes/{weapon_sub_op_key}.png').resize((23, 23))
        base_atk_mask = base_atk.copy()
    ...
    drew_base.text((1652 - score_len // 2, 420),
                   str(score_total), font=config_font(75))
    b_len = drew_base.textlength(f'{score_cv_basis}換算', font=config_font(24))
    drew_base.text((1867 - b_len, 585),
                   f'{score_cv_basis}換算', font=config_font(24))
    ...
    for i, parts in enumerate(['flower', "wing", "clock", "cup", "crown"]):
        ...
        preview_mask = Image.open(
            f'{cwd}/Assets/ArtifactMask.png').convert('L').resize(preview.size)
        preview.putalpha(preview_mask)
        if parts in ['flower', 'crown']:
            preview_paste.paste(
                preview, (-37 + 373 * i, 570), mask=preview_mask_1)
        elif parts in ['wing', 'cup']:
            preview_paste.paste(
                preview, (-36 + 373 * i, 570), mask=preview_mask_1)
        else:
            preview_paste.paste(
                preview, (-35 + 373 * i, 570), mask=preview_mask_1)
        ...
        main_op_len = drew_base.textlength(option_map.get(
            main_op) or main_op, font=config_font(29))
        drew_base.text((375 + i * 373 - int(main_op_len), 655),
                       option_map.get(main_op) or main_op, font=config_font(29))
        main_icon = Image.open(
            f'{cwd}/emotes/{main_op}.png').convert("RGBA").resize((35, 35))
        main_mask = main_icon.copy()
        base_image.paste(main_icon, (340 + i * 373 -
                         int(main_op_len), 655), mask=main_mask)

        mainv = details['main']['value']
        if main_op in disper:
            mainvsize = drew_base.textlength(
                f'{float(mainv)}%', config_font(49))
            drew_base.text((375 + i * 373 - mainvsize, 690),
                           f'{float(mainv)}%', font=config_font(49))
        else:
            mainvsize = drew_base.textlength(
                format(mainv, ","), config_font(49))
            drew_base.text((375 + i * 373 - mainvsize, 690),
                           format(mainv, ","), font=config_font(49))
        level_len = drew_base.textlength(
            f'+{details["Level"]}', config_font(21))
        drew_base.rounded_rectangle(
            (373 + i * 373 - int(level_len),
            748,
            375 + i * 373,
            771),
            fill='black',
            radius=2)
        drew_base.text((374 + i * 373 - level_len, 749),
        ...
        for a, sub in enumerate(details['sub']):
            ...
            if sub_op in ['HP', '攻撃力', '防御力']:
                drew_base.text(
                    (79 + 373 * i,
                     811 + 50 * a),
                    option_map.get(sub_op) or sub_op,
                    font=config_font(25),
                    fill=(
                        255,
                        255,
                ...
                drew_base.text(
                    ...
                    font=config_font(25))
            sub_icon = Image.open(
                f'{cwd}/emotes/{sub_op}.png').resize((30, 30))
            sub_mask = sub_icon.copy()
            base_image.paste(
                sub_icon, (44 + 373 * i, 811 + 50 * a), mask=sub_mask)
            if sub_op in disper:
                sub_size = drew_base.textlength(
                    f'{float(sub_val)}%', config_font(25))
                drew_base.text((375 + i * 373 - sub_size, 811 + 50 * a),
                               f'{float(sub_val)}%', font=config_font(25))
            else:
                sub_size = drew_base.textlength(
                    format(sub_val, ","), config_font(25))
            ...
            if details['Level'] == 20 and details['rarelity'] == 5:
                nobi = drew_base.textlength(
                    "+".join(map(str, psb[a])), font=config_font(11))
                drew_base.text((375 + i * 373 - nobi, 840 + 50 * a), "+".join(
                    map(str, psb[a])), fill=(255, 255, 255, 160), font=config_font(11))

        score = float(score_data[parts])
        atf_score_len = drew_base.textlength(str(score), config_font(36))
        drew_base.text((380 + i * 373 - atf_score_len, 1016),
                       str(score), font=config_font(36))
        drew_base.text((295 + i * 373 - atf_score_len, 1025),
                       'Score', font=config_font(27), fill=(160, 160, 160))
        ...
        score_image = score_image.resize(
            (score_image.width // 11, score_image.height // 11))
    ...
    for i, (n, q) in enumerate(set_bounus.items()):
        if len(set_bounus) == 2:
            drew_base.text((1536, 243 + i * 35), n,
                           fill=(0, 255, 0), font=config_font(23))
            drew_base.rounded_rectangle(
                (1818, 243 + i * 35, 1862, 266 + i * 35), 1, 'black')
            drew_base.text((1835, 243 + i * 35), str(q), font=config_font(19))
        if len(set_bounus) == 1:
            drew_base.text((1536, 263), n, fill=(
                0, 255, 0), font=config_font(23))
