...
def generation(data: dict):
    ...
    if CharacterName in ['蛍', '空']:
        CharacterImage = Image.open(
            f'{cwd}/character/{CharacterName}({element})/avatar.png').convert("RGBA")
    else:
        if CharacterCostume:
            CharacterImage = Image.open(
                f'{cwd}/character/{CharacterName}/{CharacterCostume}.png').convert("RGBA")
        else:
            CharacterImage = Image.open(
    ...
    if CharacterName == 'アルハイゼン':
        CharacterAvatarMask2 = Image.open(
            f'{cwd}/Assets/Alhaitham.png').convert('L').resize(CharacterImage.size)
    else:
        CharacterAvatarMask2 = Image.open(
            f'{cwd}/Assets/CharacterMask.png').convert('L').resize(CharacterImage.size)
    ...
    # 武器
    Weapon = Image.open(
        f'{cwd}/weapon/{WeaponName}.png').convert("RGBA").resize((128, 128))
    ...
    WeaponRImage = Image.open(
        f'{cwd}/Assets/Rarelity/{WeaponRarelity}.png').convert("RGBA")
    ...
    for i, t in enumerate(['通常', 'スキル', "爆発"]):
        ...
        Talent = Image.open(
            f'{cwd}/character/{CharacterName}/{t}.png').resize((50, 50)).convert('RGBA')
    ...
    for c in range(1, 7):
        if c > CharacterConstellations:
            ...
            CharaCPaste.paste(CharaC,
                              (int(CharaCPaste.width / 2) - 25,
                               int(CharaCPaste.height / 2) - 23),
                              mask=CharaCMask)
    ...
    def genbasetext(state):
        ...
        return f"+{format(plusv,',')}", f"{format(basev,',')}", D.textlength(f"+{format(plusv,',')}",
                                                                             font=config_font(12)), D.textlength(f"{format(basev,',')}", font=config_font(12))
    ...
    if WeaponSubOPKey is not None:
        ...
        D.text(
            (1623,
             155),
            f'{optionmap.get(WeaponSubOPKey) or WeaponSubOPKey}  {str(WeaponSubOPValue)+"%" if WeaponSubOPKey in disper else format(WeaponSubOPValue,",")}',
            font=config_font(23))
